...
\textoResumo{ 
Durante o levantamento do estado da arte a respeito do tema
``extração de dados estruturados da \textit{web}'', foram identificados, entre os
trabalhos mais relevantes, três  grupos  principais  de  abordagens:
extração de listas e tabelas; extração a partir de uma única página e extração a
partir de múltiplas páginas. Esta área, apesar de bastante estudada, ainda
representa um problema em aberto e também apresenta outras questões ainda não
totalmente esgotadas como, por exemplo, a utilização de métricas mais adequadas
e \textit{baseline} e/ou \textit{datasets} padronizados para facilitar a
comparação entre as diversas propostas.  Em vista dos desafios existentes, esta
pesquisa apresenta uma proposta, inédita, que aborda o problema de extração a
partir de uma única página. A abordagem converte a árvore DOM da página
\textit{web} para uma sequência de \textit{tag paths} e trata esta sequência
como um ``sinal discreto'' a partir do qual são identificadas as regiões com
conteúdo estruturado, através da utilização de técnicas de processamento de
sinais (FFT, autocorrelação, etc). A seguir é realizada a filtragem,
identificação e alinhamento dos registros das regiões identificadas.
}
...
estruturados a partir de fontes semi-estruturadas de forma completamente
...
semi-estruturada contida em páginas \textit{web}. 

Os objetivos específicos são:
\begin{itemize}
\item desenvolver um método de segmentação de um documento HTML que identifique
as regiões da página com conteúdo semi-estruturado;
\item desenvolver um método que identifique quais das regiões semi-estruturadas
possuem conteúdo e quais possuem ruído (menus, template, anúncios, etc.);
\item desenvolver um método de estruturação da informação semi-estruturada
...
apresenta a fundamentação teórica; o Capítulo \ref{ch:trab} apresenta os trabalhos
...
entendimento da abordagem proposta no Capítulo \ref{ch:prop}.
...
que é definido como $\Sigma = \{\alpha | \exists{TPS[i]=\alpha} \wedge 1 \leq i
\leq n\}$, onde $\alpha$ é um símbolo do alfabeto.
\end{definition}
...
tempo $O(nlogn)$) comumente utilizado para o cálculo da DFT (discrete Fourier
...
transformação inversa, portanto a complexidade de tempo para calculo da
autocorrelação é $O(nlogn)$.
\end{definition}
\begin{equation}\label{eq:xcorr}
C_{x_n} = IFFT(X_k \cdot X_k^*)
\end{equation}
...
frequência contribui na formação da sequência.
\end{definition}
...
potência coincide, neste caso, com o período da sequência original.  

As Definições \ref{def:fft}, \ref{def:xcorr} e \ref{def:power} foram
apresentadas neste texto de maneira breve e superficial e sua exposição foi
...
\texttt{Heur.}, \texttt{KB}, \texttt{Impl.} indicam, respectivamente: o tipo
da abordagem, se é extração de listas, tabelas, registros ou regiões; a
entrada, se é apenas uma única página ou múltiplas páginas; o alinhamento dos
registros, se é realizado ou não; a renderização da página HTML, se é necessária
ou não; regras heurísticas, se são empregadas ou não; bases de
conhecimento, se são utilizadas ou não e; se existe implementação
...
\item[Registros:] regiões com conteúdo semi-estruturado são compostas por
...
\section{Desafios e Perspectivas}\label{ch:persp}
...
\State $n \leftarrow length(regs)$
\For {$i \leftarrow 1..n$}
\State $regionCenter \leftarrow \frac{regs[i].start +
regs[i].end}{2}$
\State $positionScore \leftarrow 1-\frac{abs(tpsCenter -
regionCenter)}{maxDistance}$ \State $sizeScore \leftarrow
\frac{length(regs[i])}{tpsSize}$ \State $score[i] \leftarrow \frac{positionScore+sizeScore}{2}$
\EndFor
