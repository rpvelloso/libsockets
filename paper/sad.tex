...
\textoResumo{A extração de dados estruturados, apesar de ser um assunto bastante
estudado, ainda representa um problema em aberto e também apresenta outras
questões ainda não totalmente esgotadas como, por exemplo, a utilização adequada
das métricas e \textit{baselines} e/ou \textit{datasets} padronizados para
facilitar a comparação entre as diversas propostas. Durante o levantamento do
estado da arte a respeito do tema ``extração de dados estruturados da
\textit{web}'', foram identificados, entre os trabalhos mais relevantes, três
grupos  principais  de  abordagens:
...
\chapter{Introdução}
A extração de dados estruturados da web vem sendo estudada já há algum
tempo\footnote{Em \cite{RRunner01}, publicação mais antiga considerada de
relevância nesta pesquisa, são mencionados trabalhos de extração estruturada
publicados entre os anos de 1997-1999} em função de sua importância, que se deve
à grande quantidade de informação estruturada disponível para extração que, de
outra forma, não poderia ser manipulada de maneira relacional (utilizando SQL,
por exemplo), apenas textual (ou não estruturada). Os objetivos que motivam a
extração dessas informações são inúmeros (enriquecer pesquisas por
palavra-chave, comparação de preços, meta-pesquisa, etc.), assim como os
desafios envolvidos na realização desta tarefa como, por exemplo, identificar
as regiões de dados, identificar os registros, alinhar os campos dos registros,
integrar os dados, entre outros.
De acordo com \cite{structured2011}, a extração de dados estruturados é uma
tarefa árdua, pois as informações são provenientes de diversas fontes e são
publicadas de maneira desorganizada.
...
propostas várias abordagens para cada formulação, como se pode verificar no
Capítulo \ref{ch:trab}, onde são apresentados vários trabalhos de extração,
cada um sugerindo uma abordagem distinta para o problema.
...
% utilizadas, é necessário identificar a qual entidade do mundo real os dados se
% referem e quais atributos da entidade estão representados nas instâncias
% identificadas.  Para realizar esta tarefa, a utilização de taxonomias,
% ontologias, bases de conhecimento e afins, é imprescindível.
...
chaves utilizando as relações, construir bases de dados para
determinados domínios (e.g., comparação de preços de produtos), realizar meta-pesquisa, entre
outras aplicações \cite{webtables2008, relationalWeb2008, 2015webtables}.
...
formas distintas de abordagem que abrangem os trabalhos mais relevantes nesta
área:
extração de tabelas/listas, extração de registros, dedução de \textit{template}.

Em \cite{listExtract2009,topklists2013,tegra2015}, por exemplo, o problema é
tratado como extração de tabelas e/ou listas. São abordagens com escopo mais
limitado, que tem por objetivo extrair apenas tabelas formatadas com a \textit{tag}
...
que funcionem a contento nos mais diversos \textit{templates} de \textit{web
sites} e domínios.
Devido à enorme quantidade de dados, a precisão do processo de extração deve ser
...
possuem conteúdo e quais possuem ruído (menus, \textit{template}, anúncios,
etc.);
...
extração estruturada. A técnica utiliza uma representação alternativa do
...
\label{def:tps} 
Uma sequência de \textit{tag paths} (TPS) de uma árvore DOM com ``n'' nós é
definida como sendo a sequência ordenada $TPS[1..n] = (TP_1,TP_2,TP_3,
...,TP_{n-1},TP_n)$ onde dois \textit{tag paths} $TP_i$ e $TP_j$, com
$i\neq{}j$, são considerados iguais apenas se seus caminhos e suas definições de
estilo são iguais, caso contrário eles são considerados diferentes. Os caminhos
da árvore DOM são percorridos em profundidade e para cada caminho é atribuído um
código, de maneira incremental.
\end{definition}
\textbf{Exemplo.} Na Figura \ref{fig:ex1} é exibido o processo de construção da
$TPS$ de um trecho de código HTML, onde cada $TP$ recebe um código, incremental,
produzindo $TPS = (1,2,3,4,4,3,4,4)$.
...
uma sequência é cíclica, com período ``$p$'', \textit{i.e.}, se ela se parece
com ela mesma quando deslocada de ``$p$''. A autocorrelação de uma sequência
pode ser calculada de forma eficiente utilizando-se a FFT, como mostra a Equação
\ref{eq:xcorr}, onde $X_k$ é a FFT de $x_n$, $X_k^*$ é o conjugado complexo de
$X_k$ e a autocorrelação $C_{x_n}$ de $x_n$ é igual a transformada inversa do
produto de $X_k$ com seu conjugado complexo. O produto $(X_k \cdot X_k^*)$ é
...
\end{equation}
\textbf{Exemplo.} A Figura \ref{fig:fft}a apresenta uma região estruturada
(cíclica) de uma página \textit{web} real (Figura \ref{fig:tps}) cuja
autocorrelação é exibida na Figura \ref{fig:fft}b. Como a sequência é cíclica, sua
...
|a + i \cdot b|^2=(\sqrt{a^2 + b^2})^2=a^2 + b^2
\end{equation}
\textbf{Exemplo.} A Figura \ref{fig:fft}a apresenta uma região estruturada
(cíclica) de uma página \textit{web} real (Figura \ref{fig:tps}) cujo espectro  
de potência é exibido na Figura \ref{fig:fft}c. O componente de frequência com maior
...
Durante o levantamento do estado da arte foram identificadas duas tendências
principais de pesquisa: uma delas, mais adotada pela indústria (\textit{e.g.},
...
academia se preocupa com problemas mais gerais\footnote{\textit{e.g.}, extrair
tabelas independente das \textit{tags} utilizadas na sua formatação}, muitas
vezes produzindo resultados intermediários, que podem servir para o desenvolvimento de
trabalhos futuros. Em contrapartida, as pesquisas desenvolvidas pela academia
podem não produzir resultados com aplicabilidade comercial imediata, diferente
do que ocorre, geralmente, na indústria.

Uma questão problemática identificada durante o levantamento foi a maneira como
são realizadas as comparações entre as diversas propostas existentes devido,
principalmente, aos seguintes fatores:
\begin{itemize}
    \item a maioria das propostas não teve sua implementação disponibilizada publicamente;
    \item os \textit{datasets} utilizados são diferentes, não existe padronização e, muitas vezes, 
    também não são disponibilizados publicamente \cite{survey2013}, como no caso
    dos trabalhos desenvolvidos pela indústria que dependem de uma quantidade maciça de dados;
    \item as métricas normalmente utilizadas (\textit{recall}, precisão e \textit{f-measure}) 
    não são adequadamente empregadas na medição da eficácia dos
    trabalhos de extração estruturada e alinhamento de registros, conforme
    exposto em \cite{ariex2016}.
\end{itemize}
...
da tabulação dos dados. Uma desvantagem desta abordagem é que são necessários ao
menos dois registros em uma mesma página para que o padrão seja passível de
identificação.
...
STEM (\textit{Suffix Tree-based data record Extracting Method}). O algoritmo
proposto processa páginas HTML individuais. A árvore DOM é convertida para a mesma
...
(\textit{i.e.}, que não necessite de treinamento ou ``renderização'' da página e
tenha complexidade de tempo polinomial), que seja baseada em observações gerais
e não em regras heurísticas ou bases de conhecimento que comprometam sua
generalidade e sua validade no longo prazo. Além desses atributos, para permitir
a replicação e facilitar a comparação e a análise das propostas, estas devem ser
disponibilizadas publicamente (bem como os \textit{datasets}). Em muitos casos
não é possível sequer implementar uma abordagem, de forma independente, a partir
da sua publicação, pois faltam informações e detalhes indispensáveis para tanto.
A proposta apresentada no Capítulo \ref{ch:prop} foi elaborada tendo em vista
estes requisitos.
...
Seção \ref{sec:persp}, e em seguida, com base em sua análise, é apresentada uma
proposta para abordagem do problema de extração estruturada, composta das etapas
de: limpeza do código HTML e construção da árvore DOM (Seção \ref{sec:dom});
construção da sequência de \textit{tag paths} (Seção \ref{sec:tps}); contorno da
sequência, derivada e segmentação (Seção \ref{sec:contour}); filtragem das
regiões (Seção \ref{sec:filter}); identificação e alinhamento dos registros
(Seção \ref{sec:align}).
...
A extração estruturada pode ser composta por diversas etapas: extração,
identificação do \textit{schema}, integração e consulta. Nesta pesquisa o foco foi dirigido
...
Diante do levantamento bibliográfico realizado, foi constatado que existem,
ainda, muitos problemas em aberto nesta área de pesquisa e, até o presente
momento, não existe nenhum consenso com relação a nenhuma das etapas do processo
...
(no Capítulo \ref{ch:trab} podem ser encontradas situações desse tipo).
...
alinhamento de sequências de DNA \cite{seqalignwiki}), poucas soluções
foram propostas especificamente para área de \textit{data mining web}
\cite{viper05,depta05,datapath2015}.
Não é raro que particularidades de um problema específico acabem levando a soluções melhores,
...
si, existe espaço, também, para propostas de \textit{benchmarking} para esta
área de pesquisa, pois conforme apontado em levantamentos recentes
...
incrementalmente. A Figura \ref{fig:tps} apresenta a construção da TPS de uma
página \textit{web} real, esta TPS é utilizada nas próximas seções para
ilustrar cada etapa do processo de extração.
...
\begin{figure}[h]
  \centering
     \includegraphics[angle=0,width=\textwidth]{img/creat-tps-pt.jpg}
  \caption{TPS construída a partir de uma página \textit{web} real; região
  principal destacada.}
  \label{fig:tps}
\end{figure}
...
contorno da sequência, conforme Algoritmo \ref{alg:contour}. O contorno é o
valor máximo encontrado na sequência até cada posição. Como os códigos dos
\textit{tag paths} (os símbolos) são \textbf{incrementais}, a sequência sempre
avança para um novo valor máximo cada vez que um novo caminho é encontrado, e
nas regiões com registros a sequência oscila, pois os registros possuem
estruturas semelhantes e, portanto, compartilham os mesmos símbolos, ou seja, os
símbolos se repetem nas regiões estruturadas do documento, como ilustrado na
Figura \ref{fig:contour}.

O motivo pelo qual o contorno é calculado é que nas regiões estruturadas ele é
plano, pois essas regiões oscilam, mantendo o valor máximo, ao passo que em
...
\begin{figure}[H]
  \centering
     ...
     210},clip,width=\textwidth]{img/contour-pt.pdf} \caption{Contorno da TPS,
     calculado com Algoritmo \ref{alg:contour}.}
  \label{fig:contour}
\end{figure}
...
\begin{figure}[h]
  \centering
     \includegraphics[trim={55 200 40
     210},clip,width=\textwidth]{img/derivada-pt.pdf}
...
de cada região e analisado seu coeficiente angular. A idéia é utilizar a
regressão linear da região para determinar qual a tendência da sua TPS,
\textit{i.e.}, se a sequência está avançando/subindo, então a reta terá
inclinação/angulação elevada significando, neste caso, que o conteúdo não é
estruturado; ou, então, se a sequência é cíclica, então a reta terá angulação
reduzida indicando, portanto, que seu conteúdo é estruturado.
Se a inclinação da reta for maior que um determinado limiar a região é
descartada por não possuir conteúdo estruturado. Este procedimento é adotado,
pois mesmo após a segmentação da sequência utilizando a derivada do contorno,
ainda é possível que algumas regiões identificadas não possuam conteúdo
estruturado. O coeficiente angular da regressão linear é uma boa medida para
identificar conteúdo estruturado, pois as regiões estruturadas são cíclicas (ou
...
exemplificado na Figura \ref{fig:fft}a.
...
centro (onde estão as regiões com maior \textit{score}) é considerado como
conteúdo e as regiões do \textit{cluster} com menor centro são descartadas.
Supõe-se que exista ao menos uma região de dados.
...
Uma vez que as regiões que contêm registros foram identificadas é necessário
encontrar os pontos onde essas regiões podem ser subdivididas em registros.
Para realização desta tarefa é necessário que se conheça a \textbf{quantidade de
registros} e o \textbf{tamanho dos registros}.

Para encontrar estimativas razoáveis para estes valores (quantidade e tamanho de
registros), são investigadas as propriedades cíclicas e de frequência da TPS da
região. Especificamente, é calculada a autocorrelação e o espectro de potência
para cada região.

Como as regiões estruturadas são cíclicas, o \textbf{período} de cada uma dessas
regiões corresponde ao \textbf{tamanho dos registros} que elas contêm e a
\textbf{frequência} associada a este período corresponde à \textbf{quantidade
de registros}, conforme ilustrado na Figura \ref{fig:fft}.
O Algoritmo \ref{alg:period} apresenta o cálculo da estimativa de tamanho e
quantidade de registros (período e frequência), conforme descrito a seguir.
...
até aquele ponto, \textit{e.g.}, se existe uma autocorrelação significativa com
deslocamento igual $30$, pode significar que os registros tem um tamanho médio
de $30$ posições e uma quantidade (ou frequência) aproximada de $\frac{n}{30}$,
onde $n$ é o tamanho total da região.
Desta forma, os picos da autocorrelação são ordenados do maior para o menor
valor de correlação (Figura \ref{fig:fft}b) e seus respectivos períodos são
convertidos para frequência (Figura \ref{fig:fft}c) para verificação contra o
espectro de potência da região. O ponto de autocorrelação que possuir
correspondência com o maior pico no espectro de potência é adotado como sendo
o valor mais provável do tamanho e quantidade dos registros, pois tanto seu
valor de autocorrelação quanto sua potência são relativamente elevados.
...
analisados do menor para o maior (ordem ascendente), pois seus códigos são
atribuídos \textit{incrementalmente}, o que significa que quanto menor o código
de um símbolo, antes ele surgiu na sequência (e, portanto, está mais próximo da
raiz da árvore DOM). Ao favorecer símbolos com valores menores (analisando-os
em ordem ascendente), evita-se que um símbolo contido no meio dos registros
(e não no início) seja utilizado para subdivisão da região, comprometendo o
...
aquele símbolo, como demonstrado nas Equações \ref{eq:scoreSize},
...
\begin{figure}[h]
  ...
  \caption{a) TPS e regressão linear da região; b) autocorrelação; c) FFT e
  estimativas de quantidade e tamanho dos registros.}
  \label{fig:fft}
\end{figure}
...
\State $powerSpectrum \leftarrow |fftRegion|^2$
...
\pagebreak
\section{Cronograma}
As próximas etapas e o cronograma desta pesquisa são as seguintes:
\begin{enumerate}
  \item \textbf{levantamento bibliográfico}: leitura de artigos, teses e
  dissertações para levantamento do estado da arte (até 12/2018);
  ...
  para comparação com as demais abordagens existentes (até 12/2016);
  \item \textbf{publicação dos artigos}: escrita e publicação de artigos em
  congressos e periódicos (até 12/2017);
  \item \textbf{disponibilização do \textit{framework}}: finalização da codificação do
  \textit{framework} C++/Lua \cite{luahome}, documentação da API e disponibilização publica (até 12/2017);
  \item \textbf{qualificação}: defesa do exame de qualificação (até 6/2017);
  \item \textbf{defesa}: finalização da escrita da tese e apresentação da defesa final (até 12/2018).
\end{enumerate}
