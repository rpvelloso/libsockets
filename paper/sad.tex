...
*** falar do framework que foi construido para extração

\section{Limpeza do código HTML e construção da árvore DOM}
...
integrada ao sistema de extração. O HTML é convertido para XHTML e a árvore DOM
é construída a partir daí.
...
\section{Construção da sequência de \textit{tag paths} (TPS)}
...
incrementalmente.
O Algoritmo \ref{alg:tree2seq} \texttt{convertToSeq()} detalha como é construída
a sequência.
...
\section{Contorno da sequência, derivada e segmentação}
...
se repetem nas regiões estruturadas do documento, conforme ilustrado em
\ref{fig:contour}.
...
símbolos (ou alfabeto). Caso as regiões compartilhem parte do alfabeto com
regiões adjacentes, estas são combinadas para formar uma única região, conforme
...
região é descartada por não possuir conteúdo estruturado. Mesmo após a
segmentação da sequência utilizando a derivada do contorno, ainda é possível que
algumas regiões identificadas não possuam conteúdo estruturado. O coeficiente
angular da regressão linear é uma boa medida para identificar conteúdo
estruturado, pois essas regiões são cíclicas (ou estáveis na terminologia
utilizada em processamento de sinais) e, portanto, sua regressão linear será uma
reta com coeficiente angular próximo de zero.

O segundo filtro é para identificar se a região, apesar de ter conteúdo
estruturado, não se trata de "ruído" (menus, ads, rodapé, etc). Para identificar
estas regiões é calculado um score para cada uma das regiões a partir da sua
posição na sequência (que é correlacionada com sua posição na tela - SUPOSIÇÃO)
e do seu tamanho em relação ao documento como um todo. As regiões são
...
\section{Identificação e alinhamento dos registros}
...
Cada ponto da autocorrelação corresponde a um deslocamento da sequência em
relação à ela mesma. Os pontos com maior correlação são possíveis períodos, pois
significam que a sequência tem uma correlação maior com ela mesma deslocada.
Assim, os picos da autocorrelação são ordenados do maior para o menor e o
período é convertido para frequência para verificação contra o espectro de
frequência. O período da autocorrelação que corresponder ao maior pico no
espectro de frequência é adotado como sendo o valor provável do tamanho dos
registros.

Após a apuração do valor mais provável do tamanho dos registros, os símbolos da
região são analisados para encontrar qual deles melhor divide a sequência em
registros. O \textit{score} de cada símbolo é calculado a partir do período e
percentual total da região coberto por aquele símbolo, em relação à estimativa,
caso seja selecionado. Conforme ilustrado em \ref{}.
