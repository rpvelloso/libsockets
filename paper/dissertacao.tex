...
para este fim, na sua maioria ou requerem definições \textit{a priori} de regras
estáticas e/ou bases de dados \cite{vips03}, ou são supervisionadas (não
automáticas) \cite{Graph08}, ou então dependem de aspectos específicos da
linguagem \abreviatura{HTML}{hypertext markup language}HTML para funcionar, como
em \citeonline{Entropy09}.
...
Trata-se de um algoritmo de segmentação das regiões de um documento HTML, de
forma geral, utilizado aqui no contexto de extração estruturada, levando em
consideração o estilo e a estrutura da página. As principais contribuições são as seguintes:
...
combinado com o algoritmo proposto, como ilustrado na Figura
\ref{fig:method}, obtendo-se uma média de 88,86\% de ruído removido das páginas do conjunto de
teste.
...
três categorias: as que utilizam o texto da página (sem considerar a estrutura
do documento), as que utilizam a árvore \abreviatura{DOM}{document object
model}DOM e as que utilizam informação visual (o documento renderizado).
...
\item Em \citeonline{Densiometric08}, é mencionada a desvantagem das abordagens
visuais, devido à grande complexidade computacional das mesmas, pois precisam
renderizar a página para obter as informações de $layout$. Neste trabalho
\cite{Densiometric08} a página é modelada como uma seqüência de blocos de texto
separados por $tags$ (chamados de $gaps$), os blocos de texto são analisados
segundo sua densidade textual e os blocos contíguos, com aproximadamente a mesma
densidade, são agrupados para formar blocos maiores e a região principal da
página seria a de maior densidade textual.

\item Em \citeonline{Boilerplate10}, a mesma medida de densidade textual
proposta em \citeonline{Densiometric08} é utilizada, porém uma série de outras
características são agregadas ao problema como: o tamanho das palavras, tamanho
das sentenças, numero de palavras e a densidade de $links$, além de um conjunto
de regras bem específicas para tratar textos e $tags$ do documento. Estas
outras informações agregadas são utilizadas para gerar uma árvore de decisão
utilizada para identificar o conteúdo principal do documento.
...
atuais. Neste trabalho, em cada linha do documento, é calculada a taxa de
$tags$, que consiste na relação entre o número de caracteres que não são $tag$ e
o número de $tags$ existentes em uma determinada linha do documento, essas
linhas são, então, agrupadas de acordo com esta informação e classificadas como
sendo parte do conteúdo principal ou não.
\end{itemize}
...
partir de amostras de páginas anotadas manualmente. Os nós da árvore DOM são os
nós do grafo e os pesos das arestas representam o custo de se segmentar os dois
nós, que estão interligados, juntos ou separados. Apenas a questão da
segmentação é abordada, sem a preocupação em identificar quais partes da
página contém conteúdo relevante e quais contém ruído.

\item Em \citeonline{Entropy09}, uma árvore ``visual'' é gerada a partir da
árvore DOM, dividindo a página em blocos, com o auxílio de um dicionário de
$tags$ pré-definido. Cada nó da árvore visual tem sua entropia calculada com
relação ao atributo $CLASS$, utilizando três metodologias distintas: entropia
dos nós filhos (\textit{child entropy}), entropia do caminho (\textit{path
entropy}) e entropia do caminho parcial (\textit{partial path entropy}). Após o
cálculo da entropia, estes valores são analisados, comparados à um limiar
estipulado e cada bloco da página é, então, classificado como: CB
(\textit{container block}), PB (\textit{presentation block}), FB
(\textit{functional block}) ou IB (\textit{informative block}, conteúdo relevante).

\item \citeonline{SiteOriented11} é uma abordagem orientada a $site$, assim como
\citeonline{Noisy03}, onde um conjunto de árvores DOM de páginas de um mesmo
$site$ são alinhadas para identificar os segmentos de uma página a partir das
semelhanças entre elas, o resultado do alinhamento é, então, refinado, de acordo
com regras heurísticas, e o resultado final representa a
subdivisão/organização/$template$ do $site$ analisado pelo algoritmo. Este
...
A entropia dos demais nós é calculada recursivamente, a partir da entropia dos nós
descendentes. Quanto maior for esta medida de entropia, maior a probabilidade de
se tratar de uma região de interesse.
...
coordenadas de tela, definições de estilo, etc.). A relação custo-benefício
(complexidade computacional $versus$ ganho na precisão) de se
utilizar um renderizador para obtenção destes dados teria que ser melhor
estudada.
...
\item \citeonline{viper05} trata-se de uma técnica de extração estruturada, com
identificação do conteúdo/ruído. Utiliza o renderizador de um navegador $web$
...
padrões, onde estes recebem um peso, inversamente proporcional à distância que
se encontram do centro da página. Outra melhoria proposta, relativa à
identificação de padrões dos registros, é o tratamento de repetições
consecutivas (\textit{tandem repeats}) na comparação da estrutura dos registros.
Esta abordagem identificada o ruído apenas após a etapa de extração, sem
evitar o custo computacional de procurar por padrões no ruído.
...
\item \citeonline{vide10} é, na verdade, uma abordagem para extração estruturada
que, assim como \citeonline{BlockImp07}, se baseia em \citeonline{vips03} para
criar a representação visual do documento e, então, utilizar as características
visuais das páginas da $web$ oculta para localizar e extrair os registros
contidos no documento. As características visuais utilizadas para identificação
do conteúdo principal são:
...
Na Tabela \ref{table:comp} são comparadas as diversas técnicas de remoção de
ruído com relação às seguintes características:
\begin{itemize}
  \item Se operam sobre o texto da página, árvore DOM, informações visuais ou
  TPS;
  \item Se necessitam de apenas uma página para funcionar ou de um conjunto de
  páginas;
  \item Se necessitam de treinamento e/ou supervisão;
  \item Se necessitam de informações pré-definidas;
  \item Se dependem, ou não, da sintaxe HTML.
\end{itemize}
...
\#14). Por exemplo: as técnicas que não dependem da sintaxe HTML, necessitam de
treinamento e/ou definições \textit{a priori}. A abordagem adotada em
\citeonline{viper05} atende à todos os requisitos, no entanto, utiliza um
...
página $web$ e identificação da região principal. O algoritmo proposto é
simples, computacionalmente eficiente e leva em consideração a estrutura e o
estilo de formatação das páginas. A página $web$ é representada na forma de
$tag$ $paths$.
É um algoritmo de segmentação genérico, utilizado aqui no contexto da extração
estruturada.
