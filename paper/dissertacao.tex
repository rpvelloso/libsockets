...
região da TPS que possui um alfabeto único a, finalmente; $pruneDOMTree()$, na Linha
$5$, poda na árvore DOM todos os nós que não estão na TPS resultante,
preservando a estrutura do documento retornado na Linha $6$.
...
\section{$convertToSeq()$ Algorithm}
...
representa a $string$ do tag path anterior (da chamada recursiva anterior).
Na Linha $2$, o tag path anterior é concatenado com a $tag$ atual, assim como com suas
definições de estilo, com objetivo de distinguir caminhos repetidos com estilos diferentes; 
na Linha $3$, é verificado se o $tag$ $path$ atual já foi visto antes pelo algoritmo ou não 
($tagPathMap$ is used for this purpose) e, se não, na Linha $4$, ele é inserido no 
conjunto $tagPathMap$ e um novo código é atribuído à ele na Linha $5$,
como definido na Definição \ref{def:tps}; na Linha $7$, o código do tag path é concatenado ao final da sequencia e,
finalmente, o procedimento e invocadorecursivamente na Linha $9$ para cara filho de $node$.

\section{$searchRegion()$ Algorithm}
...
A Figura \ref{fig:alg} apresenta uma ilustração da execução do algoritmo com
objetivo de facilitar o entendimento. Na ilustração foi omitido o procedimento
de filtragem dos símbolos com baixas freqüências para maior clareza, pois a
...
\ref{ch:probform};  in Line \ref{line:tpsit} the TPS is iterated; in Line
...
\section{$pruneDOMTree()$ Algorithm}
...
\section{Algorithm's complexity}
