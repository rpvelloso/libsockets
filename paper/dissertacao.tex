...
\section{Técnicas baseadas no texto}\label{sec:txt}
Em \citeonline{Densiometric08,Boilerplate10,CETR10,BlockImp07,NonTemplate13} a
...
Densiometric08: 
	- cita a desvantagem das abordagens visuais (alto custo computacional/alta
	complexidade);
	- a página é modelada como uma seqüência de blocos de texto separados por tags
	(gaps), os blocos de texto são analisados segundo sua densidade textual,
	blocos com a mesma densidade são agrupados para formar blocos maiores e a
	região principal da página seria a de maior densidade textual.

Boilerplate10:
	- utiliza a densidade textual proposta em Densiometric08 agregando uma
	série de outras características ao problema como: o tamanho das
	palavras, tamanho das sentenças, numero de palavras e a densidade de links, além de 
	um conjunto de regras bem específicas para tratar texto e tags do documento;
	Utiliza algoritmos de árvores de decisão para calcular o ganho de informação
	alcançado com cada uma das características e com suas combinações.
	
CETR10:
	- fala da desvantagem de utilizar regras vinculadas a tags HTML específicas;
	- para cada linha do documento, é calculada a taxa de tags que consiste na
	relação entre o número de caracteres que não são tag e o número de tags
	existentes em uma determinada linha do documento, essas linhas são, então,
	agrupadas de acordo com esta informação e classificadas como conteúdo ou não;
		
\section{Técnicas baseadas na árvore DOM}\label{sec:dom} 
Em \citeonline{Entropy09,SiteOriented11,Graph08,Noisy03,Entropy12,desc2013} a
...
desc2013:
- utiliza, de forma semelhante à Boilerplate10, a densidade de texto e de links,
porém o documento é tratado como uma árvore DOM enão como uma sequencia de
blocos; além destas características, são utilizados os campos de formulários da
web oculta para treinar uma árvore de decisão utilizando algoritmo C4.5 para
identificar quais nós são ruído na resposta do formulário.

Entropy12:
utiliza uma base de dados de termos associados a ``papéis semânticos'' para
encontrar, árvore DOM, os nós folha que desempenham estes papéis, atribuindo a
cada um uma medida de ``entropia semântica-estrutural'', de acordo com o papel
desempenhado pelo nó folha. A entropia dos demais nós é calculada
recursivamente, a partir da entropia dos nós descendentes. Quanto maior esta
medida de entropia, maior a probabilidade de se tratar de uma região de
interesse.

Noisy03:
constroi uma arvore de estilos para um determinado site, a partir de várias
amostras de páginas, para determinar o que é ruído e o que é conteúdo nas
páginas daquele site determinado. A árvore de estilos do site é construída e o
ruído é determinado com base na seguinte observação feita pelos autores: os
blocos de ruído possui formatação e conteúdo semelhantes, enquanto que o
conteúdo é diverso e possui formatação diversa.

Graph08:
o problema de segmentar uma página é modelado como um problema de minimização de
energia em cima de um grafo ponderado construído a partir de amostras de páginas
anotadas. Os nós da árvore DOM são os nós do grafo e os pesos das arestas
representam o custo de se segmentar os dois nós interligados juntos ou
separados. Apenas a questão da segmentação é abordada, não há preocupação em
identificar quais partes da página contém conteúdo relevante e quais
contém ruído.
	
\section{Técnicas baseadas em informação visual}\label{sec:vis}
...
vips03:
utiliza um renderizador de um navegador web para obter informação visual
(principalmente coordenadas e formatação) de uma página e, assim, construir uma
estrutura de árvore que representa a hierarquia dos blocos visuais do documento.
Além da informação visual também é utilizado um extenso conjunto de regras que
servem para dar tratamento diferenciado a tags HTML específicas com objetivo
principal de identificar separadores visuais. Neste trabalho apenas, assim como
em \citeonline{Graph08}, não há preocupação em identificar quais blocos
visuais da página contém conteúdo relevante e quais contém ruído.

vide10:
É, na verdade, uma abordagem para extração estruturada que se baseia no trabalho
...
utilizadas para identificação do conteúdo principal são: posição (centralização
horizontal), tamanho (conteúdo é maior que demais blocos), layout (registros
possuem arranjo semelhante), semelhança visual (registros possuem formatação
semelhante).

viper05:
Trata-se de uma técnica de extração estruturada, com identificação do
conteúdo/ruído. Utiliza o renderizador de um browser para corrigir (no caso de
HTML mal formado) e processar o HTML e para obter as informações visuais
necessárias ao algoritmo.
...
consecutivas (tandem repeats) na comparação da estrutura dos registros.
...
6 & \citeonline{Noisy03} & DOM & Não & Sim & Não & Sim \\
7 & \citeonline{Graph08} & DOM & Não & Sim & Não & Sim \\
8 & \citeonline{Entropy09} & DOM & Sim & Não & Sim & Não \\
9 & \citeonline{SiteOriented11} & DOM & Não & Sim & Não & Sim \\
10 & \citeonline{Entropy12} & DOM & Sim & Não & Sim & Não \\
11 & \citeonline{Jointop07} & DOM & Não & Sim & Não & Sim \\
12 & \citeonline{desc2013} & DOM & Não & Sim & Não & Sim \\
13 & \citeonline{vips03} & Visual & Sim & Não & Sim & Não \\
14 & \citeonline{viper05} & Visual & Sim & Não & Não & Não \\
15 & \citeonline{vide10} & Visual & Sim & Não & Sim & Não \\
\hline
...
\begin{itemize}
  ...
  páginas;
  \item Se necessitam de treinamento/supervisão;
  \item Se necessitam de informações pré-definidas;
  \item Se dependem, ou não, da sintaxe HTML.
\end{itemize}

Como é possível observar, na Tabela \ref{table:comp}, as técnicas
existentes para eliminação de ruído, aplicáveis à extração estruturada
(Linhas \#6 a \#15) não atendem a todos os requisitos que o método proposto
atende (Linha \#16). Os métodos que não dependem da sintaxe HTML, por exemplo,
necessitam de treinamento e/ou definições \textit{a priori}.
