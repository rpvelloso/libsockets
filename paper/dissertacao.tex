...
seqüência de \textit{tag paths} (TPS) como representação da página \textit{web}.
...
Um passo fundamental da mineração de dados na \textit{web}, inclusive em extração
...
organizados em listas, que freqüentemente descrevem objetos similares''.
...
freqüentemente contém padrões em sua estrutura, fazendo com que sejam
identificados como registros e, conseqüentemente, extraídos prejudicando a
...
direcionados para indexação e \textit{clustering} de páginas (\textit{i.e.} é
assumido que a região principal é textual/não estruturada) como em
...
a seqüência de \textit{tag paths} da árvore DOM. Conforme a análise apresentada
na Seção \ref{sec:compl}, o algoritmo é computacionalmente eficiente. Trata-se,
em sua forma mais ampla, de um algoritmo de segmentação das regiões de um
documento HTML que é utilizado aqui no contexto de extração estruturada e que
...
a segmentação das partes de uma página e subseqüente identificação da região
...
Em \citeonline{Densiometric08} a página é modelada como uma seqüência de
...
segmentação, e subseqüente identificação da região principal, é realizada
utilizando a árvore DOM da página, portanto estas técnicas levam em consideração
a estrutura da página \textit{web} e, conseqüentemente, podem ser aplicadas no contexto
...
Em \citeonline{Graph08} o problema de segmentar uma página é modelado como
...
\#14). As técnicas que não dependem da sintaxe HTML, necessitam de treinamento
e/ou definições \textit{a priori}. A abordagem adotada em \citeonline{viper05}
atende a todos os requisitos, no entanto, utiliza um renderizador (que é
computacionalmente dispendioso) e só identifica o ruído posteriormente à
identificação dos registros, desperdiçando uma oportunidade de otimizar o
processo, identificando e eliminando o ruído antes de submeter a página à
extração.
...
Estes dois trabalhos são citados aqui apenas para ilustrar que, de acordo com
seus resultados, esta representação, assim como a árvore DOM, também é capaz de
...
\begin{figure}[h]
  ...
  \caption{Exemplo de uma seqüência de \textit{tag paths} construída a partir do código HTML.}
  \label{fig:ex1}
\end{figure}
...
\textit{tag paths}, chamada de TPS (seqüência de \textit{tag paths}) e seu
respectivo alfabeto (conjunto de símbolos da seqüência).
...
\begin{definition}[\textbf{Seqüência de \textit{\textit{tag paths}}}]\label{def:tps}
Uma seqüência de \textit{tag paths} (\abreviatura{TPS}{tag path sequence}TPS) de uma
...
\textit{tag path} é representado na seqüência por um símbolo, exceto que neste trabalho
foram incorporadas as definições de estilo das \textit{tags}. Na Figura \ref{fig:ex1}
...
uma seqüência de \textit{tag paths}.
...
símbolos de baixa freqüência da $TPS$, não seria possível identificar essas
...
\begin{algorithm}[h]
\caption{Converte uma árvore DOM para a representação de seqüência de \textit{tag path}}
\label{alg:tree2seq}
...
$7$, o código do \textit{tag path} é concatenado ao final da seqüência e, finalmente,
...
seqüência onde é possível dividi-la (\textit{i.e.} onde existe uma região); na Linha
...
realização.
Para obter os resultados apresentados na Seção \ref{sec:res}, o algoritmo
proposto foi implementado e testado em vários \textit{web sites} comerciais
e institucionais da maneira descrita na Seção \ref{sec:setup}. Na Seção
\ref{sec:clar}, um destes resultados é detalhado, para exemplificar e esclarecer
como os dados foram obtidos e compilados na Tabela \ref{table:results}.

\section{\textit{Setup} dos Experimentos}\label{sec:setup}
Os experimentos foram realizados em páginas da \textit{web}, em HTML, de \textit{sites} que
apresentam conteúdo estruturado. São páginas de diversos domínios, de
empresas/instituições que são referência em seus segmentos de atuação. O
conjunto de dados foi previamente salvo em disco para acesso \textit{offline}.

Para medir a performance do algoritmo foram utilizadas as medidas de praxe em
...
qualidade total da mineração de dados apresenta ganho, devido aumento maior na
precisão.
