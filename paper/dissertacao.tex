...
utilizando a árvore DOM da página, portanto estas técnicas levam em consideração
a estrutura da página \textit{web} e, consequentemente, podem ser aplicadas no contexto
...
Neste capítulo, é apresentada, primeiro, na Seção \ref{sec:reptps}, a forma de
representação dos documentos HTML utilizada neste trabalho (TPS). Na Seção
...
as frequências de $FS$. Seja $FT = \{f|\exists{(s,f)} \wedge (s,f) \in FS \wedge
s \in \Sigma \}$, onde ``s'' é um símbolo do alfabeto $\Sigma$ e ``f'' é sua
...
a $11$; na Linha $12$, os limiares de frequência, como especificado Definição
