...
$tag$ $paths$}
% Titulo do trabalho \subtitulo{Estilo \LaTeX~ padrão}                %
% Subtitulo do trabalho (opcional)
\autor{Roberto Panerai Velloso}
\data{22}{maio}{2013}                           % Data da publicação do
\orientador{Profa. Dra. Carina F. Dorneles}                    % Nome do
\begin{document}
\capa \folhaderosto[comficha] % Se nao quiser imprimir a ficha, é só não usar o
% parâmetro 
%\folhaaprovacao 
%\paginadedicatoria{Dedicado à Xuxa} 
%\paginaagradecimento{Muito obrigado.}
%\paginaepigrafe 
\listadefiguras 
\listadetabelas 
%\listadeabreviaturas
...
seqüência $tag$ $paths$ (TPS) como representação da página $web$. A TPS é
composta por uma seqüência de símbolos ($string$), cada um representando um
$tag$ $path$ diferente. A técnica proposta procura por posições na TPS onde é
...
fully automatic algorithm that uses a tag path sequence (TPS) representation
of the web page. The TPS consists of a sequence of symbols (string), each one
...
identifying the main content block of several major web sites, and improves
the precision of the extraction step by removing irrelevant results.
}

\paginaresumo \paginaabstract
...
trabalhos nesta área e, de acordo com \citeonline{Editorial04}, o ruído existe
nas páginas pode prejudicar consideravelmente a mineração de dados na $web$.
...
HTML para funcionar, como em \citeonline{Entropy09}.

Este trabalho propõe e descreve um algoritmo para segmentação e remoção de ruído
de páginas $web$ que utiliza a representação de seqüência de $tag$ $paths$ de
uma página e que é simples e computacionalmente eficiente.
...
Para avaliar o quão efetiva é a abordagem proposta neste trabalho, foi comparado
o ruído de saída do MDR \cite{MDR03}, uma conhecida técnica de extração
estruturada, contra o ruído de saída do MDR combinado com a técnica proposta,
como ilustrado na Figura \ref{fig:method}, obtendo-se uma média de 84,87\% de
ruído removido das páginas do conjunto de teste.

\begin{figure}[H]
  \centering
    \includegraphics[scale=0.44]{method.jpg}
...
\ref{ch:contr} são enunciadas as principais contribuições do trabalho; no
Capítulo \ref{ch:rev} é feita uma revisão dos trabalhos relacionados, observando
as limitações das propostas existentes para solução do problema; no Capítulo
\ref{ch:def} são enunciadas as definições necessárias ao entendimento da
proposta; no Capítulo \ref{ch:probform} é descrita a proposta deste trabalho e
as hipóteses nas quais a mesma se baseia; no Capítulo \ref{ch:alg} são
detalhados e ilustrados os algoritmos desenvolvidos a partir da formulação do
problema e das hipóteses; no Capítulo \ref{ch:met} o método de avaliação dos
resultados é detalhado; no Capítulo \ref{ch:results} são apresentados os
resultados obtidos e; no Capítulo \ref{ch:conclusion} uma conclusão.
...
páginas que possibilite incrementar a precisão dos métodos atuais, com o mínimo
...
para funcionar, o que é um grande vantagem, como discutido em
\citeonline{Editorial04}.
\end{itemize}
...
árvore DOM e as que utilizam informação visual.
...
\citeonline{SiteOriented11} e
\citeonline{Noisy03} necessitam de várias páginas de um mesmo $site$,
\citeonline{Graph08} propõe um $framework$ de treinamento que necessita de
dados catalogados manualmente para funcionar e \citeonline{Entropy12} necessita
de uma base de dados de termos associados a ``papéis semânticos'' para poder
detectar regiões de dados.
...
\begin{figure}[h]
  \caption{Exemplo de uma seqüência de $tag$ $paths$ construída a partir do
  código HTML.}
  \label{fig:ex1}
  \centering
...
Na Tabela \ref{table:comp} é feito um comparativo entre as diversas técnicas
revistas e a proposta apresentada, com objetivo de explicitar as diferenças
...
\hline
Fernandes et al. [2007] & Texto & - & - & - & - \\
Kohlschütter and Nejdl [2008] & Texto & - & - & - & - \\
Kohlschütter et al. [2010] & Texto & - & - & - & - \\
Weninger et al. [2010] & Texto & - & - & - & - \\
Hu et al. [2013] & Texto & - & - & - & - \\
Yi et al. [2003] & Árvore DOM & Não & Sim & Não & Não \\
Chakrabarti et al. [2008] & Árvore DOM & Não & Sim & Não & Não \\
Cho et al. [2009] & Árvore DOM & Sim & Não & Sim & Sim \\
Fernandes et al. [2011] & Árvore DOM & Não & Sim & Não & Não \\
Zheng et al. [2012] & Árvore DOM & Sim & Não & Sim & Sim \\
Zheng et al. [2007] & Árvore DOM & Não & Sim & Não & Não \\
Cai et al. [2003] & Rep. visual & Sim & Não & Sim & Sim \\
Simon and Lausen [2005] & Rep. visual & Sim & Não & Não & Sim \\
Liu et al. [2010] & Rep. visual & Sim & Não & Sim & Sim \\
\hline
Proposta & TPS & Sim & Não & Não & Não \\
\end{tabular}
\end{tiny}
\end{table}
...
\begin{definition}[\textbf{$tag$ $paths$}]\label{def:tp} Um $tag$ $path$ (TP) é
uma $string$ descrevendo o caminho absoluto, a partir da raiz da árvore DOM, até
um dado nó da árvore. Seja ``i'' a posição em profundidade do nó $``n_i"$ na
...
Uma seqüência de $tag$ $paths$ (TPS) de uma árvore DOM com ``n'' nós é definida
como sendo a seqüência ordenada $TPS[1..n] = (TP_1,TP_2,TP_3, ...,TP_{n-1},TP_n)$
...
\section{Algoritmos}\label{ch:alg}

Neste capítulo são apresentados os algoritmos desenvolvidos para tratar o
problema definido no Capítulo \ref{ch:probform}. Quais sejam:

\begin{itemize}
  \item {}\textit{\textbf{tagPathSequenceFilter()}}. É o algoritmo
  ...
  \item {}\textit{\textbf{convertToSeq()}}. Converte a árvore DOM de uma página
  $web$ para sua representação $TPS$;
  \item {}\textit{\textbf{searchRegion()}}. A busca, propriamente dita, pela
  região principal da $TPS$;
  \item {}\textit{\textbf{filterAlphabet()}}. Filtra um alfabeto, removendo 
   símbolos de menor freqüência, tornando o algoritmo de busca mais robusto e
resistente ao ruído;
  \item {}\textit{\textbf{pruneDOMTree()}}. Poda da árvore DOM original,
...
\begin{figure}[H]
  ...
  \label{fig:alg}
  \centering
    \includegraphics[scale=0.40]{alg-pb.jpg}
\end{figure}
...
Definição \ref{def:alpha}, é filtrado, como descrito no Capítulo
\ref{ch:probform}; na Linha $22$ a TPS é iterada; na Linha $25$ o alfabeto da
...
\textbf{Input:} $symCount$ - o conjunto das freqüências ($FS$) dos $tag$ $path$ em $\Sigma$ \\ 
...
em $currentAlphabet$ são considerados durante a busca pela região principal.

\subsection{Algoritmo $pruneDOMTree()$}

\begin{algorithm}[H]
\caption{Poda da árvore DOM todos os nós que não pertencem à seqüência}
\label{alg:prune}
...
8 & Bondfaro & 3897 & 3069 & -21,25\% & Sim & 231 & 28 & 178 & 28 & 26,11\% \\
9 & Bradesco & 1913 & 1113 & -41,82\% & Sim & 164 & 10 & 93 & 10 & 46,10\% \\
10 & Build & 2712 & 746 & -72,49\% & Não & 117 & 12 & 69 & 0 & N/A \\
11 & Buscapé & 3608 & 3514 & -2,61\% & Sim & 279 & 24 & 266 & 24 & 5,10\% \\
...
removeu, com este conjunto de teste, uma média de 84,87\% de todo o ruído
presente nos dados, como mostra a média da coluna ``Ruído rem.''.

A média de 48,52\% de redução no tamanho da árvore DOM é um resultado
...
\begin{enumerate}
  ...
  pode ser que seja ruído). Mais a frente, neste capítulo, é proposta uma forma
  de contornar este problema;
  \item{\textbf{páginas com região principal menor que o restante}}. Este
  problema é uma conseqüência da segunda hipótese levantada no Capítulo 
  \ref{ch:probform}: ``a região principal é mais densa, ou maior, que as demais
...
oito casos - 17,65\%), quando a região principal é menor que as demais, uma
abordagem semântica também pode ser utilizada para validar se a região encontrada, de
fato, possui conteúdo de interesse e, se não possuir deve ser descartada e a
árvore complementar ($i.e.$ inverter a poda) deve ser retornada em seu lugar. O
algoritmo principal ficaria assim:
...
observado um incremento considerável na precisão da extração.

Nas situações em que nosso algoritmo atua aquém das expectativas, outras
