...
\item{Extraction optimization}: the proposed algorithm prunes and average of 46.22\% of the DOM tree, in linear time, avoiding the processing of this noise by the subsequent extraction algorithm.  
\end{itemize}
...
\State $current\Sigma \leftarrow filter(\Sigma,symCount,thresholds[t])$
\If {$current\Sigma.size < 2$}
\State $break$
\EndIf
\State $currentSymCount \leftarrow symCount$
\State $region\Sigma \leftarrow \emptyset$
\For {$i \leftarrow 1..n$}
\State $sym \leftarrow TPS[i]$
\If {$sym \in current\Sigma$}
\State $region\Sigma \leftarrow region\Sigma \cup \{sym\}$
\State $currentSymCount[sym] \leftarrow currentSymCount[sym]-1$
\If {$currentSymCount[sym] = 0$}
\State $current\Sigma \leftarrow current\Sigma - \{sym\}$
\If {$current\Sigma \cap region\Sigma = \emptyset$}
...
\textbf{Input:} $symCount$ - the tag path frequency set ($FS$) of the
alphabet \\ 
\textbf{Input:} $threshold$ - a frequency threshold \\
\textbf{Output:} a filtered alphabet
\begin{algorithmic}[1]
\Procedure{filter}{alphabet, symCount, threshold}
\State $filtered \leftarrow \emptyset$
\For {$i \leftarrow 1..n$}
\If {$symCount[alphabet[i]] \geq threshold$}
\State $filtered \leftarrow filtered \cup \{alphabet[i]\}$
\EndIf
\EndFor
...
\bibliography{jidmb}  % sigproc.bib is the name of the Bibliography in this case
