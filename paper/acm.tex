...
\EndFor
\State $thresholds \leftarrow OrderedFrequencySet(symCount)$
\State $regionFound \leftarrow false$
\While {not $regionFound$}
\State $t \leftarrow t + 1$
\State $cur\Sigma \leftarrow filter(\Sigma,symCount,thresholds[t])$
\If {$cur\Sigma.size < 2$}
\State $break$
\EndIf
...
\State $filtered\Sigma \leftarrow filtered \cup \{\Sigma[i]\}$
\EndIf
\EndFor
