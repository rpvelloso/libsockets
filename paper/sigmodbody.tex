...
Figure \ref{fig:stru} illustrates a web page converted to its sequence
representation and highlights its structured main content region as well. We
will use this specific sequence and its main content region throughout Section
\ref{sec:content} to characterize the features. The sequence was constructed
according to the definitions in this section. Every point in the sequence
(Definition \ref{def:tps}) corresponds to a specific node in the DOM tree and
has a TPCode value (Definition \ref{def:tpc}) that encodes the node's Tag Path
(Definition \ref{def:tp}).
