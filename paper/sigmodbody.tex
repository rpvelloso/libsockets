...
density (PSD).
Figure \ref{fig:fft}(a) shows the PSD of the main content region in Figure
\ref{fig:fft}(b) and its detected record count and average record size. The
sequence in Figure \ref{fig:fft}(a) is the extracted main content region from
the sequence depicted in Figure \ref{fig:stru}.
In this example the number of records (represented by the peak in Figure
\ref{fig:fft}(a)) detected is 20 and their average size is 31, so the value of
the feature, using Equation \ref{eq:recsize}, is equal do
$\frac{20}{31}=64.51\%$
