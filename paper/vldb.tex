...
used to identify it inside a document and improve its structure to the point
...
the web became what it is today (e.g., FFT\cite{fft1965} algorithm as published
in 1965) and as a consequence it is much more developed in that aspect, so it is
only natural to employ some of these consolidated techniques when we succeeded
in converting the problem of record extraction into a signal processing problem.
...
finite difference (Definition \ref{def:diff}) and within each of this cyclic
...
represents the number of records within that data region) and then use this
information (record size and record count) to break down the data region into
records that are subsequently aligned.
...
have exposed to the Lua environment.
%(there is also an integrated relational database available to the user -- SQLite\cite{sqlite}).
...
Dataset	& \#1	& \#2 \\ \hline
Sites &	51 & 10 \\ \hline
Pages per site	& 5 & 3 \\ \hline
Average records & 21 & 22 \\ \hline
Total records &	1052 & 218 \\ \hline
\end{tabular}
%}
\end{table}
