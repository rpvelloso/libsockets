...
calculated using the fast Fourier transform\cite{fft1965} (FFT for short --
Definition \ref{def:fft}).
The Fourier transform factors the sequence into its frequency components, so
...
get the average record size (31 in this case).
...
detected for that region (highlited with squares). In this case the TPCode with
value 252 matches the spectrum and CV constraints as we shall see. TPCode 252
...
the number of occurences (frequency) and the average size (period) match the
peak encoutered in the region's spectrum at position 20 and the CV for this
