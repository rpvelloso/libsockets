...
approaches found in the literature that we consider relevant.  
...
summarized results. Although several works criticizes this aspect, few provide
implementations.

We have implemented our approach using a two tier software architecture where
the core in coded in C++ and the presentation layer (graph plotting, result
manipulation, logging, timing, etc.) in embedded Lua \cite{}. Our
implementation is publicly available at \cite{} and we encourage the scientific
community to build on top of it, run independent experiments, implement other
techniques in parallel and so on.

We used two datasets for comparison. Dataset $#1$ was proposed by
\cite{yamada2004testbed} and was used by \cite{TPC09,clustVX2014}. Dataset $#2$
was used in \cite{clustVX2014}.

Table \ref{table:compare1} compares precision, recall and F-Measure of our
approach with three other state-of-the-art techniques: MDR\cite{MDR03},
TPC\cite{TPC09} and ClustVX\cite{clustVX2014}. Our approach performs better than
MDR and TPC and slightly worst than ClustVX, but our technique doesn't need
to render the entire page in order to extract the records.
...
\begin{table}[h]
\centering
\caption{Results comparison using the dataset from \cite{clustVX2014}.}
