...
\renewcommand{\ABNTbibliographyname}{REFERENCES}
...
\titulo{Exame de Qualificação de Doutorado: Extracting Records from the Web Using a Signal Processing Approach}
...
\textoResumo{Extracting records from web pages enables a number of important
...
\chapter{Related Work}\label{sec:work}
...
grigalis2013towards, autorm2015} more general approaches are
proposed.
...
techniques\cite{RRunner01,fivatech2010,grigalis2013towards}, others
...
% diferencial do trabalho: complexidade de tempo baixa, independe do HTML, nï¿½o
% renderiza pï¿½gina,

\chapter{Preliminaries}\label{sec:defs}
...
\chapter{Structured Data Extraction}\label{sec:prop} We present here, in this
...
\begin{figure}[H]
  \centering
     \includegraphics[width=\textwidth]{img/proposal.jpg}
  \caption{\small{Extraction system overall diagram.}}
  \label{fig:overall}
\end{figure}

\section{Sequence Representation}\label{ss:seq}
...
\begin{figure}[H]
  \centering
     \includegraphics[width=\textwidth]{img/tree2seq.jpg}
...
\section{Region Identification}\label{ss:regi}
...
\section{Record Identification}\label{ss:reci}
...
input region.
\section{Record Alignment}\label{ss:reca}
...
\chapter{Experiments}\label{sec:result}
...
and so on.
 ...
 SQLite\footnote{\url{https://sqlite.org/}}).
...
\chapter{Schedule}
The schedule for the next steps on this research are the following:
\begin{enumerate}
  \item \textbf{update state-of-the-art survey}: paper and thesis reading
  (until 12/2018);
  \item \textbf{experiments and results}: improve over the current experiments
  and results, refine implementation (until 12/2018);
  \item \textbf{paper publishing}: publish another paper
  with regard to record alignment (until 06/2018);
  \item \textbf{co-advise master thesis}: co-advise master thesis that follows
  up this research. The thesis consists of replacing FFT $O(nlogn)$) by
  Goertezel's algorithm ($O(n)$) to improve time complexity (until 12/2018);
  \item \textbf{thesis}: present PhD thesis (until 12/2018);
\end{enumerate}

\apendice
\chapter{Extraction Framework Usage Example}\label{app:a}
