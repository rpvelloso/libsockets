...
%In \citeN{Editorial04}, it is mentioned that noise can seriously harm web data mining.
In \citeN{Noisy03}, it is mentioned that despite the importance of this task, relatively 
...
\citeN{Editorial04}, noise can seriously harm web data mining. 

In structured extraction, most of the existing approaches use some sort of
pattern recognition to identify the records (as defined in \citeN{MDR03}) present
...
\citeN{SiteOriented11,Noisy03} and, due to intrinsic differences between
...
language to work, such as \citeN{Entropy09}.

In this article, we propose and lay down a simple, computationally efficient, yet
...
\item{Works on single page}: it is required only one page as input, which is a main advantage as discussed in \citeN{Editorial04};
...
%MDR \citeN{MDR03}, a well known structured extraction technique, with and without
...
MDR \citeN{MDR03}, a well known structured extraction technique, against the output
of MDR combined with our technique, as illustrated in Figure \ref{fig:method}, yielding 
an average of 77.03\% of noise removed from the test web pages.
...
This article is organized as follows. In Section \ref{sec:relatedwork}, we give a
brief survey of related work in segmentation, pointing out the differences
...
\citeN{BlockImp07,Densiometric08,Boilerplate10,CETR10,NonTemplate13} the segmentation is done
...
\citeN{Noisy03,Graph08,Entropy09,SiteOriented11,Entropy12,Jointop07} the segmentation is done using
the DOM tree and, thus, they do take into account the web page's structure.
However \citeN{SiteOriented11} and \citeN{Noisy03} require several pages from the
same web site, they are site-driven techniques, \citeN{Graph08} and \citeN{Jointop07} proposes a
training framework that requires a manually labeled data set to work, \citeN{Entropy09}
is dependent of a tag dictionary, defined a priori, to build a
visual representation of the page and \citeN{Entropy12} requires a database of terms associated
...
\citeN{vips03,viper05,vide10}. They all rely on a web browser's renderer to
obtain the visual information used for segmentation, what can be
computationally expensive, and beyond that, \citeN{vips03} is based on quite
...
\citeN{NET05,RRunner01,MDR03,TPC09,SuffixTree12}, just to name a few. The reason we 
...
also employed in \citeN{TPC09} and \citeN{SuffixTree12}, although in both cases
...
\end{definition}
This is the same definition as in \citeN{SuffixTree12}, where each different tag
...
equal to \textbf{1}, symbol ``3'' has frequency equal to \textbf{2} and symbol ``4''
has frequency equal to \textbf{4}. The $FT$ set is need to filter out
...
Given the definitions presented in the previous section, we formulate next the
...
\citeN{MDR03}), the main region is structurally denser than the others (menus, ads,
text, etc.).
\end{enumerate}
...
$region$ in the $TPS$ of a web page that has an $alphabet$ that does not intersect
...
\section{Algorithms' description}\label{sec:algorithm}
...
to keep it simple and easy to understand the main idea behind the $searchRegion()$ algorithm.
...
this number matches the value reported, independently, by \citeN{Volume05} as
...
%techniques, like the one presented in \citeN{Adaptive07}, that labels a sequence 
