...
Em \citeonline{Boilerplate10}, a mesma medida de densidade textual
...
Em \citeonline{Entropy09}, uma árvore ``visual'' é gerada a partir da
...
Em \citeonline{viper05} é proposta uma técnica de extração estruturada,
...
Em \citeonline{BlockImp07}, é utilizada a abordagem de \citeonline{vips03}
...
Na Seção \ref{sec:probform} o problema é enunciado a partir das definições e das
hipóteses levantadas. Na Seção \ref{sec:alg} são apresentados os algoritmos
...
institucionais da maneira descrita na Seção \ref{sec:setup}. Na Seção
...
apresentam conteúdo estruturado. O conjunto de dados foi previamente salvo em
disco para acesso \textit{offline}. Para medir a performance do algoritmo foram
utilizadas as medidas de praxe em \textit{data mining}: precisão ($precision$),
revocação ($recall$) e \textit{F-measure}. A medida \textit{F-measure} é importante
para evidenciar que, mesmo em situações com diminuição de $recall$, ainda assim a
...
evitaria o processamento do ruído. Essas medidas são particularmente relevantes
...
da árvore DOM (coluna 'Red.' da Tabela \ref{table:results}) e percentual de
registros espúrios removidos (coluna 'Ruído rem.' da Tabela
\ref{table:results}), calculado conforme a Fórmula \ref{eq:noise}. Ambos são
detalhados na Seção \ref{sec:clar}.
