...
atuais. Neste trabalho, em cada linha do documento, é calculado a taxa de
...
Nos trabalhos detalhados nesta seção
\cite{Entropy09,SiteOriented11,Graph08,Noisy03,Entropy12,desc2013} a segmentação
é realizada utilizando a árvore DOM da página, portanto, estas técnicas levam em
consideração a estrutura da página $web$ e podem ser, portanto, aplicadas no
contexto de extração estruturada.

\begin{itemize}
\item Em \citeonline{desc2013} de forma semelhante à \citeonline{Boilerplate10},
são utilizadas as densidades de texto e de $links$, porém o documento é tratado como
uma árvore DOM e não como uma seqüência de blocos. Além destas características,
são utilizados os dados dos formulários da $web$ oculta para formar mais
um parâmetro ($QueryScore$) de treinamento da árvore de decisão com o
algoritmo C4.5 \cite{quinlan1993c4}. A árvore de decisão gerada é utilizada para
identificar quais nós da árvore DOM são ruído e quais são conteúdo relevante na
resposta à submissão do formulário.
...
semântica-estrutural'', de acordo com o papel desempenhado pelo nó folha. A
entropia dos demais nós é calculada recursivamente, a partir da entropia dos nós
...
algumas que utilizam informação visual, como \citeonline{vips03,vide10,viper05}.
...
\item \citeonline{vide10} é, na verdade, uma abordagem para extração
estruturada que se baseia em \citeonline{vips03} para criar a representação
visual do documento e, então, utilizar as características visuais das páginas da
$web$ oculta para localizar e extrair os registros contidos no documento. As
características visuais utilizadas para identificação do conteúdo principal são:
