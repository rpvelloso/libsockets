...
combinado com o algoritmo proposto, como ilustrado na Figura \ref{fig:method}. O
conjunto de testes consiste em uma coleção de 51 páginas de $sites$ com conteúdo
estruturado em diversos domínios como: vídeos, buscas, comércio eletrônico de
vários segmentos, notícias, periódicos científicos, bancos, entre outros. O
resultado obtido foi uma média de 88,86\% de ruído removido das páginas do
conjunto de teste.
...
blocos de texto separados por $tags$ (chamados de $gaps$), os blocos de texto
são analisados segundo sua densidade textual e os blocos contíguos, com
aproximadamente a mesma densidade, são agrupados para formar blocos maiores e a
...
estudada. Em \citeonline{Densiometric08}, é mencionada a desvantagem das abordagens
visuais, devido à sua grande complexidade computacional das mesmas, pois
...
\begin{itemize}
  ...
  principal, que recebe um arquivo HTML como entrada, chama os demais algoritmos
  e retorna uma árvore DOM, podada, apenas com o conteúdo da região principal;
  \item{\textbf{$convertToSeq()$}} converte a árvore DOM de uma página
  $web$ para sua representação $TPS$;
  \item{\textbf{$searchRegion()$}} realiza a busca, propriamente dita, pela
...
intenção é passar a idéia principal. Os detalhes e minúcias podem ser
verificados na listagem do algoritmo e na sua descrição.
...
Para analisar a complexidade do algoritmo, é suficiente 
