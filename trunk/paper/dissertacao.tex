...
a 'segmentação das partes de uma página é realizada utilizando o conteúdo
textual do documento. O foco principal destes trabalhos, entretanto, não é em
extração estruturada, mas em indexação e $clustering$ de páginas. A maioria dos trabalhos
existentes sobre segmentação e remoção de ruído, que atuam sobre o texto, é
direcionada para este tipo de aplicação, cujo objetivo principal é identificar o
``assunto'' da página.
...
palavras e a densidade de $links$, além de um conjunto de regras bem específicas
para tratar textos e $tags$ do documento. Estas outras informações agregadas são
utilizadas para gerar uma árvore de decisão utilizada para identificar o
...
Em \citeonline{Noisy03} é construída uma árvore de estilos (com base
...
Em \citeonline{SiteOriented11} é adotada uma abordagem orientada a $site$,
...
Em \citeonline{desc2013} de forma semelhante à \citeonline{Boilerplate10},
...
Em \citeonline{vips03} é utilizado um renderizador de um navegador $web$
...
identificação do conteúdo/ruído que utiliza o renderizador de um navegador $web$
...
$score$ de importância é atribuído a cada bloco. Apesar de necessitar de um
conjunto de páginas para treinamento, toda a análise do conjunto é realizada de
forma não supervisionada.

Em \citeonline{vide10} é proposta uma abordagem para extração estruturada
...
2 & \citeonline{Boilerplate10} & Texto & - & - & - & - & Decision tree\\
3 & \citeonline{CETR10} & Texto & - & - & - & - & Clustering e limiar\\
...
9 & \citeonline{desc2013} & DOM & Não & Sim & Não & Sim & Decision tree\\
10 & \citeonline{vips03} & Visual & Sim & Não & Sim & Não & Conjunto de regras\\
11 & \citeonline{viper05} & Visual & Sim & Não & Não & Sim & MDR e Distância \\
...
em \citeonline{viper05} atende a todos os requisitos, no entanto, utiliza um
...
A representação da página $web$ utilizada no presente trabalho foi anteriormente
empregada em \citeonline{TPC09} e \citeonline{SuffixTree12}. Em ambos os casos,
esta representação foi usada para a extração e não para segmentação e remoção de
ruído.
...
expor a estrutura da página $web$ e, portanto, serve para os propósitos dos
algoritmos discutidos no presente trabalho.
...
de uma página $web$ e identificação da região principal. O algoritmo proposto leva em
consideração a estrutura e o estilo de formatação das páginas e é
computacionalmente eficiente pois, conforme demonstrado mais adiante, na Seção
...
hipótese, como é possível observar, é geral o suficiente para poder ser reutilizada em
...
Os experimentos foram realizados em páginas da $web$, em HTML, de $sites$ que
...
\textit{data mining}: precisão ($precision$), revocação ($recall$) e
\textit{F-measure}. A medida \textit{F-measure} é importante para evidenciar
que, mesmo em situações com diminuição de $recall$, ainda assim a qualidade
total da mineração de dados apresenta ganho, devido aumento maior na precisão.
...
eliminação de rúidos, foi utilizado o algoritmo MDR \cite{MDR03} é uma conhecida
técnica de extração estruturada. O resultado da extração pura (apenas MDR) será
considerado como $baseline$ para comparação com o resultado da extração
...
discutidas e tratadas na Seção \ref{sec:disc}, e, apesar desta redução, as
medidas F e G apresentaram melhora significativa.

\begin{table}[H]
\centering
\caption{Precisão, $recall$ e F-$measure$.}
\label{table:prec}
...
comprometer os registros de interesse. Também, quando aplicada em conjunto com o MDR, foi
