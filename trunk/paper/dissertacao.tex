...
frequentemente contém padrões em sua estrutura, fazendo com que sejam
...
\label{table:comp}
\begin{tabular}{p{0.1cm}|p{3cm} p{0.5cm} p{0.5cm} p{0.5cm} p{1cm} p{0.5cm}
p{1cm}} \hline\hline
\# 
&Técnica   
&Entrada
...
4 & \citeonline{Noisy03} & DOM & Não & Sim & Não & Sim & Heurística\\
%11 & \citeonline{Jointop07} & DOM & Não & Sim & Não & Sim \\
5 & \citeonline{Graph08} & DOM & Não & Sim & Não & Sim & Grafos \\
6 & \citeonline{Entropy09} & DOM & Sim & Não & Sim & Não & Entropia\\
7 & \citeonline{SiteOriented11} & DOM & Não & Sim & Não & Sim & Heurística\\
8 & \citeonline{Entropy12} & DOM & Sim & Não & Sim & Sim & Entropia Semântica\\
9 & \citeonline{desc2013} & DOM & Não & Sim & Não & Sim & \textit{Decision tree}\\
10 & \citeonline{vips03} & Visual & Sim & Não & Sim & Não & Conjunto de regras\\
11 & \citeonline{viper05} & Visual & Sim & Não & Não & Sim & Distância \\
12 & \citeonline{BlockImp07} & Visual & Não & Sim & Não & Não & Estatística\\
13 & \citeonline{vide10} & Visual & Sim & Não & Sim & Não & \textit{Clustering}\\
\hline
14 & \citeonline{TPS2013} & TPS & Sim & Não & Não & Sim & Segment. TPS\\
\end{tabular}
\end{tiny}
\end{table}
...
\#14). As técnicas que não dependem da sintaxe HTML ou necessitam de treinamento
...
%representa a página \textit{web}.
%\end{definition}
%Na Figura \ref{fig:ex1} um pequeno trecho de código é utilizado para ilustrar a
%árvore DOM e as próximas definições.

\begin{definition}[\textbf{\textit{tag paths}}]\label{def:tp} Um \textit{tag path}
(\abreviatura{TP}{tag path}TP) é uma $string$ descrevendo o caminho absoluto, a
partir da raiz da árvore DOM, até um dado nó da árvore. Seja ``$n_i$'' um
determinado nó da árvore DOM, onde ``i'' representa a posição/ordem, em
...
\begin{definition}[\textbf{Sequência de \textit{\textit{tag paths}}}]\label{def:tps}
Uma sequência de \textit{tag paths} (\abreviatura{TPS}{tag path sequence}TPS) de uma
árvore DOM com ``n'' nós é definida como sendo a sequência ordenada $TPS[1..n] = (TP_1,TP_2,TP_3, ...,TP_{n-1},TP_n)$
onde dois \textit{tag paths} $TP_i$ e $TP_j$, com $i\neq{}j$, são considerados iguais
apenas se seus caminhos e suas definições de estilo são iguais, caso
contrário eles são considerados diferentes.
\end{definition}
Na sequência de \textit{tag paths} os
\textit{tag paths} não são representados pelas suas \textit{strings}, mas sim
por códigos (ou símbolos), com objetivo de facilitar sua manipulação, onde
\textit{tag paths} iguais recebem o mesmo código. Esta definição é similar à
utilizada em \citeonline{SuffixTree12}, onde cada \textit{tag path} é representado 
na sequência por um símbolo, exceto que neste trabalho
foram incorporadas as definições de estilo das \textit{tags}. Na Figura
\ref{fig:ex1} é mostrada a $TPS$ de um dado trecho de
código HTML, onde cada $TP$ recebe um código, produzindo $TPS = (1,2,3,4,4,3,4,4)$.

\begin{definition}[\textbf{Alfabeto da TPS}]\label{def:alpha} Seja $\Sigma$ o
conjunto contendo todos os símbolos de uma dada sequência $TPS$ de tamanho
``n'', $\Sigma$ é dito o alfabeto de $TPS$ definido como $\Sigma =
\{\alpha | TPS[i]=\alpha \wedge 1 \leq i \leq n\}$, onde $\alpha$ é
...
\begin{definition}[\textbf{Conjunto de frequências de $tag$
...
símbolo ``s'' ocorre nesta TPS, então o conjunto de frequências $FS$ de uma
...
\begin{definition}[\textbf{Limiares de frequência}]\label{def:ft}
Dada uma $TPS$ com alfabeto $\Sigma$ e conjunto de frequências $FS$, os
limiares de frequência $FT$ são definidos como sendo a sequência ordenada
contendo apenas as frequências de $FS$. Seja $FT = \{f|\exists{(s,f)} \wedge
(s,f) \in FS \wedge s \in \Sigma \}$, onde ``s'' é um símbolo do alfabeto
$\Sigma$ e ``f'' é sua respectiva frequência.
\end{definition}
Na TPS exibida na Figura \ref{fig:ex1}, o conjunto de frequências dos
\textit{tag paths} é $FS=\{(1,1), (2,1), (3,2), (4,4)\}$, neste caso o
conjunto dos limiares de frequência é igual a $FT=\{1,2,4\}$, pois os 
símbolos ``1'' e ``2'' tem frequência igual a
\textbf{1}, o símbolo ``3'' tem frequência igual a \textit{2} e o símbolo ``4''
tem frequência igual a \textit{4}. O conjunto $FT$ é necessário para filtrar os
símbolos da TPS. Se $FT=\{1,2,4\}$, então não faria sentido filtrar símbolos com
frequência igual a \textit{3}, pois não existem símbolos na sequência com esta
frequência.
